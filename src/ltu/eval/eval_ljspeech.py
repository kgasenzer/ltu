...
eval_file_list = ['ljspeech_pytorch_model_caption_2024_09_25T23_21_19.json'] # TODO
...
for eval_file in eval_file_list:
    if True:
        ...
        label_list = ["It is an audio deepfake, because it shows characteristics of the BigVGAN vocoder.", "It is a real recording, because it is from the LJSpeech dataset."]
        #print(label_list)
        ...
        if True:
            ...
            for label in label_list:
                ...
                if text_embed_setting == 'gpt':
                    label_dict[label] = get_gpt_embedding('sound of ' + label.lower())
                elif text_embed_setting == 'bert':
                    label_dict[label] = get_bert_embedding('sound of ' + label.lower())
        ...
        for i in bar:
            ...
            if llm_task == 'cla':
                ...
                cur_pred_list = eval_data[i]['pred'].replace('"', '').split('Audio caption')[-1]
