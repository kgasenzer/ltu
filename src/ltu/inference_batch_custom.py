...
def main(
    ...
    temp, top_p, top_k = 0.1, 0.10, 1
    # change it to your model path
    eval_mdl_path = '/home/s6kogase/seminar/ltu/src/ltu/exp/ltu_ft_toy_low_resource/checkpoint-50/pytorch_model.bin'
    #eval_mdl_path = '/home/s6kogase/seminar/trained/full_ft_2e-5_20000.bin'
    #eval_mdl_path = "/home/s6kogase/seminar/ltu/src/ltu/exp/ltu_ft_toy_low_resource_full/checkpoint-10/pytorch_model.bin"
    ...
    for eval_dataset in eval_dataset_list:
        ...
        # fsd50k multi-label classification
            ...
            with open('../../eval_data/fsd50k_eval.json', 'r') as fp:
                data_json_1 = json.load(fp)
            task_dict = {'caption': 'Close-ended question: Write an audio caption describing the sound.', 'open': ''}
        ...
        # alpaca pure language task
            ...
            with open('../../eval_data/alpaca_1000.json', 'r') as fp:
                data_json_1 = json.load(fp)
            task_dict = {'text': ''}
        if eval_dataset == 'ljspeech':
            with open('../../eval_data/ljspeech_eval.json', 'r') as fp:
                data_json_1 = json.load(fp)
            task_dict = {'caption': 'Closed-ended question: Classify the audio clip into one of the two classes real recording ("LJSpeech") or audio deepfake ("BigVGAN").'}
            #task_dict = {'caption': 'Closed-ended question: Analyze the audio and determine whether it is a real recording or an audio deepfake.'}
            task_dict = {'caption': 'Analyze the provided audio clip and determine whether it is a real recording or an audio deepfake. When making the determination, consider that audio deepfakes often lack fine-grained frequency details in higher frequency ranges, which can lead to artifacts or unnatural sound textures. Additionally, deepfakes may exhibit inconsistencies in pitch, timbre, or background noise compared to authentic recordings. Based on these characteristics, state whether the audio is real or a deepfake.'}

        import random
        random.shuffle(data_json_1)
        for task in task_dict.keys():
            ...
            for i in bar:
                ...
                if audio_path != 'empty':
                    cur_audio_input, _audio_info = load_audio(audio_path)
